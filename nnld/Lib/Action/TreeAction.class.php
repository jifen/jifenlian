...
//			$this->todayindan($ID,$LL,$RR);
		...
		$MyYJ = "</td></tr>";
//		$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='代理编号：".$UserID."|手机：".$user_tel."|QQ：".$qq."'>联系方式</a></td></tr>";
//		$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='投资金额：".$cpzj."'>投资金额</a></td></tr>";
		...
		//代理ID,0,绝对层次,显示层高,表开始,表内链接,表结束  ,级别颜色数组,所有空位表格,显示多少代理数(包括空位数)
		$wop = '';
...
//				$this->todayindan($rss['id'],$LL,$RR);
				...
				$MyYJ = "</td></tr>";
//				$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='代理编号：".$uUserID."|手机：".$uuser_tel."|QQ：".$uqq."'>联系方式</a></td></tr>";
//				$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='投资金额：".$cpzj."'>投资金额</a></td></tr>";
	...
	//			$Str = $StTab."<a href='". __URL__ ."/PuTao/ID/". $Rid ."'>代理编号：". $uUserID ."</a>". $MyYJ;
	...
	public function Tree2b(){
		...
		$fee_rs = $fck -> find();
		$id  = $_SESSION[C('USER_AUTH_KEY')];
		$point = $fck->where('uid='.$id)->field('id')->order('id asc')->find();
		if (!$point) {
			$this->error('您在B网还没有点位');
		}
		$id = $point['id'];
		$myid=$_SESSION[C('USER_AUTH_KEY')];
		$UID = (int) $_GET['ID'];
...
//		$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='投资金额：".$cpzj."'>投资金额</a></td></tr>";
		...
		$this->Tree2_MtKassb($UID, 0, $pLevel, $uLev, $Str4C0, $Str4C1, $Str4C4,  $TreeArray, $Nums);
		//代理ID,0,绝对层次,显示层高,表开始,表内链接,表结束  ,级别颜色数组,所有空位表格,显示多少代理数(包括空位数)
		$wop = '';
		$this->Tree2_showTreeb($uLev, $TreeArray, $wop);
	...
	//双轨图---生成下层代理内容
	private function Tree2_MtKassb($FatherID,$iL,$pLevel,$uLev,$Str4C0,$Str4C1,$Str4C4,&$TreeArray,$Nums){
		$ji_c = $this->ji_Color();  //级别颜色
		$kd_c = $this->kd_Color();  //是否开通
		$mi_c = $this->Mi_Cheng();  //级别名称
		if (!empty($FatherID)){
...
//				$MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='投资金额：".$cpzj."'>投资金额</a></td></tr>";
	...
	//			$Str = $StTab."<a href='". __URL__ ."/PuTao/ID/". $Rid ."'>代理编号：". $uUserID ."</a>". $MyYJ;
				$Str = $StTab."<a href='". __URL__ ."/Tree2b/ID/". $Rid ."'>". $uUserID ."</a>". $MyYJ;
				$Str4C2 = "/FID/". $Rid ."'>点击注册</a>";
				...
				if ($upLevel < $pLevel + $uLev){
					//查出来的下级的绝对层	 //上级的绝对层,显示层数
					$this->Tree2_MtKassb($Rid, $k, $pLevel, $uLev, $Str4C0, $Str4C1, $Str4C4,  $TreeArray, $Nums, $ColorUA);
				}
			}
	...
	//双轨图----生成表格内容
	private function Tree2_showTreeb($uLev,$TreeArray,&$wop){
					       //显示层高,所有空位表格,空
		for ($i = 1;$i <= $uLev;$i++){
			$Nums = $Nums + pow(2,$i);    //要显示用户的数量
		}
	...
	public function todayindan($uid=0,&$danL=0,&$danR=0){
		$fck=M('fck');
		$fee = M('fee');
		$fee_rs = $fee->field('s10,s9,i4,s2')->find();
	...
	public function Tree3(){
		...
		$fck   =  M ("fck2");
		$loginUser  = $fck->where('uid='.$_SESSION[C('USER_AUTH_KEY')])->find();
		$id = $loginUser['id'];
		$myid = $id;
		$UID = (int) $_GET['UID'];
		if (empty($UID)) $UID = $id;
		...
		$rs = $fck ->where($where)->field($field)->find();
		if($rs==false){
			$rs = $fck ->where('uid='.$UID)->field($field)->find();
		}
		...
		$bj = "style='background:". $kd_c[$isPay] ."';";  //表格背景色
		$StTab = "<table border='0' cellpadding='0' cellspacing='1' class='tu_box'><tr><td colspan='3' style='background:". $kd_c[$uLevel] .";font-weight:bold;'>";
...
//			dump($rss);
			foreach($rss as $rs){
				...
				$bj = "style='background:". $kd_c[$uLevel] ."';";  //表格背景色
				$StTab = "<table border='0' cellpadding='0' cellspacing='1' class='tu_box'><tr><td colspan='3' style='background:". $kd_c[$uLevel] .";font-weight:bold;'>";
				...
				$Str=$StTab."<a href='".__URL__."/Tree3/UID/".$Rid."'>".$UserID."</a>".$MyYJ;
	...
	public function TreeAjax() {
		...
		$ID  = (int) $_GET['UID'];
		$Mmid=$_SESSION[C('USER_AUTH_KEY')];//dump($Mmid);exit;
		$this->assign('myid',$Mmid);
		if (empty($ID))$ID = $_SESSION[C('USER_AUTH_KEY')];
		...
		//echo $all_nn=M()->getLastSql();

		$map = array();
		$map['re_id'] = $rs['id'];
		$map['is_pay'] = array('gt', 0);
		$field = "team_yj+cpzj_pv as yj";
		$res = $fck->where($map)->field($field)->order('yj desc')->select();
		/*echo '<pre>';
		print_r($res);
		echo '</pre>';*/
		...
		$get_level = '';
		$this->_getLevelConfirm($get_level);
		if($getLevel > 0){
			$xj = "[".$get_level[$getLevel]."]";
		}
		...
		$myName = $UserID."(".$username.") [".$lev."]".$cpzj_pv;
		$myTabN = "m".$UID;
		...
		$rwheres['re_id']	= $ID;
		if($id != 1){
			$rwheres['re_level']	= $rs['re_level']+1;
		}

		$trs = $fck->where($rwheres)->order('is_pay desc,pdt asc')->select();

		$zz = 1;
		foreach($trs as $rss){
			$rssid = $rss['id'];
			$rsuserid = $rss['user_id'];
			$nickname = $rss['user_name'];
			$rusername = $rss['user_name'];
			$rsagent = $rss['is_agent'];
			$rslv = $rss['u_level'];
		//	$cpzj_pvs = $rss['team_yj'];
			...
			$z_myName = $rsuserid."(".$nickname.") [".$levs."]".$cpzj_pvs;
	...
	public function ajax_tree_m(){
		...
		foreach($trs as $rss){
			...
			$z_myName = $rsuserid."(".$rusername.") [".$lev."]".$xj;
	...
	public function Treey(){
        ...
        $list = $fck2->where($map)->field($field)->order('pai_num asc')->page($Page->getPage().','.$listrows)->select();
	...
	public function changeTreeAjax()
	{
		$this->_Admin_checkUser();
  		...
  		$fck = D('Fck');
  		$where['user_id'] = $userID;
  		$field = 'id,re_path,re_id,is_pay,re_level,user_id,user_name,re_name';
  		$user = $fck->where($where)->field($field)->find();
  		if (!$user) $this->error("未找到要移动的会员！");
  		$old_re_id = $user['re_id'];
  		$old_re_name = $user['re_name'];
  		$where['user_id'] = $toUserID;
  		$toUser = $fck->where($where)->field($field)->find();
  		if (!$toUser) $this->error("未找到所要移动到的会员");
  		//处理招商专员数
  		if ($user['is_pay'] == 1){
  			$fck->where('id='.$user['re_id'])->setDec('re_nums',1);
  			$fck->where('id='.$toUser['id'])->setInc('re_nums',1);
  			// $fck->checkReNum($toUser['id']);
  		}
  		//移动的会员处理
  		$user['re_id'] = $toUser['id'];
  		$user['re_name'] = $toUser['user_id'];
  		$rePath = $user['re_path'].$user['id'].',';
  		$user['re_path'] = $toUser['re_path'].$toUser['id'].',';
  		$newRePath = $user['re_path'].$user['id'].',';
  		$levelCha = $toUser['re_level'] - $user['re_level'] + 1; //等级差
  		$user['re_level'] = $toUser['re_level']+1;
  		$fck->save($user);	//保存

  		$where = array();
  		$where['re_path'] = array('like',$rePath.'%');
  		$fck_rs = $fck->where($where)->field('id,re_path,re_level')->select();
  		foreach ($fck_rs as $re) {
  			$re['re_path'] = str_replace($rePath,$newRePath,$re['re_path']);	//替换树的上部份
  			$re['re_level'] += $levelCha;
  			$fck->where('id='.$re['id'])->save($re);
  		}

  		$change_tree = M('change_tree');
  		$data = array();
  		$data['uid'] = $user['id'];
  		$data['user_id'] = $user['user_id'];
  		$data['old_re_id'] = $old_re_id;
  		$data['old_re_name'] = $old_re_name;
  		$data['new_re_id'] = $toUser['id'];
  		$data['new_re_name'] = $toUser['user_id'];
  		$data['pdt'] = time();
  		$change_tree->add($data);
  		$this->success("转移成功！");
	}
