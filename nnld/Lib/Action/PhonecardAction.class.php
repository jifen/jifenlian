	...
	public function Codys() {
		...
		switch ($Urlsz) {
			...
			case 2;
				...
				break;
			case 3;
				$_SESSION['UrlszUserpass'] = 'MyssPaoYingTao';//购买电话卡
				$bUrl = __URL__ . '/buyCard';
				$this->_boxx($bUrl);
				break;
			default;
				$this->error('二级密码错误!');
	...
	//发放卡号
		...
		if($cid==0){
			if($cx_n>0){
                            ...
                            $where['days'] = array('eq',$cx_n);
				$lrs = $card->where($where)->find();
...
//				$card_data['is_use']=$vo[4];
				$card_data['is_sell']=0;
				$card_data['f_time']=time();
				$card_data['days']=$vo[4];
	...
	public function buyCard() {
		if ($_SESSION['UrlszUserpass'] == 'MyssPaoYingTao'){
			$card = M('card');
			$fck = M('fck');
			$fee = M('fee');
			
			$id = $_SESSION[C('USER_AUTH_KEY')];
			$fck_rs = $fck->where('id='.$id)->field('agent_cash,u_level,shoplx')->find();
			$this->assign('fck_rs',$fck_rs);

			$fee_rs = $fee->find();
                        $type_list = explode("|", $fee_rs['s4']);
                        $this->assign('type_list',$type_list);
			$this->assign('money',$fee_rs['str8']);
	...
	public function buyCardAC(){
		...
		$id = $_SESSION[C('USER_AUTH_KEY')];
		$fck_rs = $fck->where('id='.$id)->field('agent_cash,u_level,shoplx,user_id')->find();
		$fee_rs = $fee->find();
		$moneyArr = explode("|", $fee_rs['str8']);
                $money = $moneyArr[$type];
		...
		$res = $this->fafang_card($id,$fck_rs['user_id'],6);
		if($res){
			$rs = $fck->execute("update __TABLE__ set agent_cash=agent_cash-$money where id= ".$id);
		}
