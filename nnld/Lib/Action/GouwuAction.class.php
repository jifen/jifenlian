...
class GouwuAction extends CommonAction{
    ...
    public function Buycp() { //购买产品页
		...
		// 购买类型
		$pro_type = (int)$_REQUEST['pro_type'] == 2 ? 2 : 1;
		$title = $pro_type == 2 ? '重复消费商场' : '二次购物商场';
        ...
        $page_where = 'tp='.$ss_type.'&pro_type='.$pro_type;//分页条件
...
//		}
		...
		foreach ($prs as $vo){
			...
			// $gwd['pvzhi'] = $mType == 1 ?  $ids[$vo['id']]*$w_money: $ids[$vo['id']]*$z_money;
			if ($vo['pro_type'] == 2) { // 重复消费币
				$prices_agent_kt += $gwd['cprice'];
			}
			else {
				$prices_agent_xf += $gwd['cprice'];
			}
			// $prices += $gwd['cprice'];
			// $prices += $mType == 1 ? $ids[$vo['id']]*$w_money: $ids[$vo['id']]*$z_money;

			$gouwu->add($gwd);
		}
		$have_price = $prices;
		// $agentName = $mType == 1 ? 'agent_kt' : 'agent_xf';
		if($fck_rs['agent_xf'] < $prices_agent_xf){
			$this->error("您的消费币余额不足！");
			exit;
		}
		if($fck_rs['agent_kt'] < $prices_agent_kt){
			$this->error("您的重复消费币余额不足！");
			exit;
		}
		$rs = $fck->query("update __TABLE__ set agent_kt=agent_kt-".$prices_agent_kt.",agent_xf=agent_xf-".$prices_agent_xf.",`is_xf`=is_xf+1 where id=".$id);
//		$fck->addencAdd($fck_rs['id'],$fck_rs['user_id'], -$prices,22);
		if($rs !== false){
			// $fck->gwJiandianjiang($fck_rs['p_path'],$fck_rs['user_id']);
			$_SESSION["shopping"]='';
...
//            $page_where = 'UserID=' . $UserID;//分页条件
		...
		$field = 'nnld_fck.user_id,nnld_fck.nickname,nnld_product.name,nnld_product.pro_type,nnld_gouwu.*';
		...
		$this->_productClassNameConfirm($names);
	...
	public function pro_index(){
		...
		if ($_SESSION['UrlszUserpass'] == 'MyssGuanChanPin'){
			...
			if ($pro_type === null) {
				$pro_type_name = $_REQUEST['pro_type_name'];
				// var_dump($pro_type_name);
				// switch ($pro_type_name) {
				// 	case '注册购物':
				// 		$pro_type = 0;
				// 		break;
				// 	case '二次购物':
				...
				foreach ($names as $key => $value) {
					if ($pro_type_name == $value) {
						$pro_type = $key;
					}
				}
			}
			if ($pro_type != -1) {
				$map['pro_type'] = $pro_type;
			}
			$this->assign('pro_type',$pro_type);
			// var_dump($map);
	        //=====================分页开始==============================================
	        ...
	        $page_where = 'stitle=' . $title .'&pro_type=' . $pro_type;//分页条件
	        $Page = new ZQPage($count, $listrows, 1, 0, 3, $page_where);
	        //===============(总页数,每页显示记录数,css样式 0-9)
	        $show = $Page->show();//分页变量
	        $this->assign('page',$show);//分页变量输出到模板
	...
	public function pro_edit(){
		...
		if ($rs){
			...
			$this->_productClassNameConfirm($pro_name_list);
			$this->assign('pro_name_list',$pro_name_list);
...
//		}
		...
		$data['id'] = $_POST['ID'];
		$data['pro_type'] = (int)$_POST['pro_type'];
...
//		}
	...
	public function wuliu()
	{
		...
		if ($_SESSION['UrlszUserpass'] == 'MyssWuliuList'){
			...
			if(!empty($_POST['ntype'])){
				...
				if($_POST['ntype'] == 2){
					$maps['type']=array('eq',1);

				}

				if($_POST['ntype'] == 3){
					$maps['is_ok']=array('eq',0);

				}

				if($_POST['ntype'] == 4){
					$maps['is_ok']=array('eq',1);

				}

			}

			$maps['user_id']=array('eq',$fck_rs['user_id']);

			import ( "@.ORG.ZQPage" );  //导入分页类
			$count = $fahuo->where($maps)->count();//总页数
			...
			$show = $Page->show();//分页变量
			$this->assign('page',$show);//分页变量输出到模板
			$list = $fahuo ->where($maps)->page($Page->getPage().','.$listrows)->order('type asc,is_ok asc,adt desc')->select();
	...
	public function wuliuAC(){
		...
		switch ($action){
			...
			case '确定收货';
				$this->_adminLogisticsDones($XGid);
				break;
	...
	private function _adminLogisticsDones($XGid){
		//确定发货
		if ($_SESSION['UrlszUserpass'] == 'MyssWuliuList'){
			$shopping = M ('fahuo');
			...
			$where1['is_ok'] = array ('eq',0);

			$where = array();
			$where['id'] = array ('in',$XGid);
			$where['is_ok'] = array ('eq',0);
			...
			$res = $shopping->where($where)->setField($valuearray);
	...
	public function adminLogistics()
	{
		...
		if ($_SESSION['UrlszUserpass'] == 'MyssWuliuList'){
         ...
         if(!empty($_POST['UserID'])){
             $map['user_id']=array('eq',$_POST['UserID']);
			 $user=M('fck')->where($map)->find();

			 $maps['user_id']=array('eq',$user['id']);
	...
	public function adminLogistics1(){
		...
		if ($_SESSION['UrlszUserpass'] == 'MyssWuliuList'){
            ...
            foreach($list as $key => $vv){
            	$trs = $product->where('id='.$vv['did'])->find();
            	$list[$key]['name'] = $trs['name'];
            	$list[$key]['pro_type'] = $trs['pro_type'];
            }
            $this->assign('list',$list);//数据输出到模板
            //=================================================
            $title = '物流管理';
